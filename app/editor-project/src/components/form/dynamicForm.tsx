...
export default function DynamicForm<TModel extends {}>({
  ...
  return modelKeys.map((k) => {
    ...
    if (currentDef) return <Field key={currentDef.key as string} field={currentDef} initialValue={formModel[currentDef.key]} onChange={onChange} />;
  });
}
...
export function Field<TModel>({
  ...
  useEffect(() => {
    !!onChange && onChange(value, field.key.toString())
  }, [value])

  switch (field.type) {
    case FieldTypes.INPUT:
      return <InputField field={field} value={value} onChange={handleChange} />;
